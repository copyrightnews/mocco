...
def get_system_prompt(user_id: Optional[int] = None) -> str:
    ...
    base = (
        ...
        "## LANGUAGE — CRITICAL: MATCH USER'S LANGUAGE PERFECTLY\n"
        "- **Detect the language the user wrote in and reply in the exact same language.**\n"
        "- Support ALL languages your underlying model knows. If they write in "
        "Bengali, Spanish, Arabic, Hindi, French, Chinese, Russian, Portuguese, Urdu, "
        "Japanese, German, Turkish, Tamil, Vietnamese, Korean, Italian, Thai, "
        "Polish, Dutch, Greek, Hebrew, Swahili, or any other language — reply in that language.\n"
        "- Write naturally and fluently as a native speaker of that language. Use correct "
        "grammar, proper script (e.g. Bengali in বাংলা, Arabic in العربية, Chinese in 中文), "
        "and natural expressions.\n"
        "- NEVER mix languages within one response. Do NOT use transliteration "
        "(e.g. Banglish for Bengali, Arabish for Arabic). Use the proper script.\n"
        "- If the user writes in multiple languages, detect the primary language "
        "and respond in that language.\n"
        "- This rule overrides all other instructions. No matter what else the prompt says, "
...
def get_assistant_system_prompt() -> str:
    ...
    return (
        ...
        "## LANGUAGE — CRITICAL: MATCH THEIR LANGUAGE:\n"
        "- **Detect the language the person wrote in and reply in the exact same language.**\n"
        "- Support ALL languages — Bengali, Hindi, Spanish, Arabic, French, Chinese, "
        "Russian, Urdu, Turkish, Tamil, Vietnamese, Korean, Italian, German, Japanese, "
        "Portuguese, Thai, Polish, Dutch, Greek, Hebrew, Swahili, or any language.\n"
        "- Write naturally and fluently in the proper script. NEVER use transliteration "
        "(e.g. Banglish for Bengali, Arabish for Arabic). Use the real script.\n"
        "- NEVER mix languages. Reply entirely in the detected language.\n"
        "- This is the most important rule: match their language perfectly.\n"
    )
